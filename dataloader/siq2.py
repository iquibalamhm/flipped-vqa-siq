...
class SIQ2(BaseDataset):
    def __init__(self, args=None, tokenizer=None, split='train'):
        ...
        if args.speaking_turns_path:
            self.speaking_turns = pickle.load(open(args.speaking_turns_path, "rb"))
        else:
            self.speaking_turns = None
        self.speaking_turns_sampling = args.speaking_turn_sampling
    ...
    def _get_video(self, video_id, start, end):
        ...
        if self.speaking_turns is not None:
            if self.speaking_turns_sampling==True:
                ...
                if total_frames > self.max_feats:
                    ...
                    video = torch.stack(sampled_video)
